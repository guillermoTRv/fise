         ...
         $(document).ready(function(){
           $("#unidad_slc").change(function () {
                   $("#unidad_slc option:selected").each(function () {
                    elegido=$(this).val();
                    $.post("../scripts/subir/procesar_combo_unidades.php", { elegido: elegido }, function(data){
                    $("#modelo").html(data);
                    });            
                });
           })
                ...
                <label class="control-label col-md-2">Unidad:</label>
                    <div class="col-md-6">
                        <select id="unidad_slc" class="form-control input-sm curselect" name="unidad_slc" title="Selecciona una unidad" required >
                            <option value="" >---</option>
                                <?php error_reporting(E_ALL ^ E_NOTICE); include("select_unidades.php"); ?>
                        </select>
                    </div>
            </div>
...
<?php include('../scripts/subir/recomendaciones.php'); ?>
<?php include('../scripts/subir/guiatemas.php'); ?>