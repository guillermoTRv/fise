<?php 
	if (isset($_GET['pos'])) {
			...
			$select_unidad="SELECT * FROM ficha_contenido_materia WHERE materia='$materia' and unidad='$un' order by id_fichamat desc LIMIT $init, $limit_end";
			...
			echo "<ul class='pagination'>";
				...
				if (($ini-1)==0) {
					...
					echo "<li><a href='?un=$u&pos=".($ini-1)."'><b>&laquo;</b></a></li>";
				}
				...
				for ($k=1; $k<=$total ; $k++) { 
					...
					if ($ini==$k) {
						...
						echo "<li><a href='?un=$u&pos=$k'>".$k."</a><li>";
					}
				}
				...
				if ($ini==$total) {
					...
					echo "<li><a href='?un=$u&pos=".($ini+1)."'><b>&raquo;</b></a></li>";
				}
